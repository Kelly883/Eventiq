...
return new class extends Migration
{
    ...
    {
        // Add columns first
        $hasFraudEventsUserEmail = Schema::hasColumn('fraud_events', 'user_email');
        $hasFraudEventsOrderStatus = Schema::hasColumn('fraud_events', 'order_status');
        $hasFraudEventsDeviceType = Schema::hasColumn('fraud_events', 'device_type');
        $hasFraudEventsProxyVpnDetected = Schema::hasColumn('fraud_events', 'proxy_vpn_detected');
        $hasFraudEventsIpReputationScore = Schema::hasColumn('fraud_events', 'ip_reputation_score');
        $hasFraudEventsAccountAgeDays = Schema::hasColumn('fraud_events', 'account_age_days');
        Schema::table('fraud_events', function (Blueprint $table) use ($hasFraudEventsUserEmail, $hasFraudEventsOrderStatus, $hasFraudEventsDeviceType, $hasFraudEventsProxyVpnDetected, $hasFraudEventsIpReputationScore, $hasFraudEventsAccountAgeDays) {
            // === Add missing analysis columns first ===
            // User email for dashboard display without JOIN
            if (!$hasFraudEventsUserEmail) {
            ...
            if (!$hasFraudEventsIpReputationScore) {
            ...
            if (!$hasFraudEventsAccountAgeDays) {
