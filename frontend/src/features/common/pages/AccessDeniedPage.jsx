...
const AccessDeniedPage = ({ title = 'Access Denied', message = 'You do not have permission to access this page.' }) => {
  ...
  React.useEffect(() => {
    if (location.state?.message) {
      showToast('Access Denied', location.state.message, 'warning');
    }
        ...
        <div className="flex justify-center mb-4">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-red-100 text-red-600">
            <svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m0 3.75h.008v.008H12v-.008zm.008-6.905a9 9 0 11-2.25-.241.75.75 0 00.626.825 1.5 1.5 0 10-1.5-1.5A1.5 1.5 0 0112 15z" />
            </svg>
          </div>
        </div>
