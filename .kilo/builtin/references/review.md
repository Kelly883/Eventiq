---
name: review
description: Use fresh reviewers when available. One reviewer attacks one dimension and returns findings, not rewritten product files. If only the builder is available, run the same passes sequentially and label the result self-reviewed, lower assurance.
license: MIT
metadata:
